...
def time_evolution(U, T, tau, dy, dt, nt, ny, C, gamma, M_r, Pr):
    ...
    for t in range(1, nt):
        ...
        for j in range(1, ny - 1):
            ...
            T_new[j] = T[j] + dt * ((gamma - 1) * M_r**2 * tau * U[j] + eta[j] / Pr) * (T[j + 1] - 2 * T[j] + T[j - 1]) / dy**2
...
for i, M_r in enumerate(M):
    ...
    plt.imshow(T_history, aspect='auto', origin='upper', extent=[0, 1, 0, nt * dt], cmap='inferno') #, vmin=1, vmax=1.5)
