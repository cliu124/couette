...
def read_data_fn(root_path):
  ...
  data = pinnstf2.utils.load_data(root_path, "couette_startup_M2.0_T1001_Y501_66abea8e.mat") #"couette_startup_M2.0_T1001_Y501_66a52d93.mat")
#   y = data["y"]
  U = data["U_sol"]
  T = data["T_sol"]
  return {"U": U, "T": T}
...
time_domain = pinnstf2.data.TimeDomain(t_interval=[0, 0.3], t_points = 1001)
...
def plot(vars):
    ...
    for var in vars:
        exact = mesh.solution[var]
        pred = preds_dict[var].reshape(exact.shape)
        ...
        ax.set_xlabel("$t$")
        ax.set_ylabel("$y$")
        leg = ax.legend(frameon=False, loc=(0.6, -0.25))
        ...
        ax.set_xlabel("$t$")
        ax.set_ylabel("$y$")
        ax.set_title(f"Residuals; RMSE={rmse:.4f}", fontsize=10)
        ...
        for i, point in enumerate(points):
            ...
            ax1.plot(exact[:, point], mesh.spatial_domain_mesh[:, point, 0], "b-", linewidth=2, label="Exact")
            ax1.plot(pred[:, point], mesh.spatial_domain_mesh[:, point, 0], "r--", linewidth=2, label="Prediction")
            ax1.set_ylabel("$y$")
            ax1.set_xlabel("$U$")
            ...
            ax2.plot(np.linspace(0,1,len(diff[:, point])), diff[:, point], "g--", linewidth=2, label="Residual")
            ax2.set_xlim([0, 1])
            ax2.set_ylim([-1, 1])
            if i == len(points) - 1:
        ...
        name = f"/fig-{epochs}-{round(time())}-{var}"
